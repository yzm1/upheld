# Upheld keeps a living map from software promises to defense coverage
...
Schema 0.1 calls a falsifiable obligation a `promise`. This page uses that wire term after this point.

## One map supports new designs and existing code

Upheld has two entry paths.

**Design first.** A team can write promises before code exists. It can record scope, assumptions, harm, cost limits, runtime limits, and other facts that affect defense choice. Upheld can then help form a defense plan before the feature is built.

**Audit first.** A team can scan an existing project for promises and defenses already present in docs, code, tests, types, config, and runtime checks. Upheld can then compare that current state with a stronger or cheaper defense plan.

The two paths meet in the same data. A discovered promise may later become an explicit design rule. An authored defense plan may later acquire real code and evidence.

## Each promise has a desired state and a current state
...
The main report should make the comparison easy to read. A user should be able to move from “this promise has a gap” to the exact reason and the next useful action.
...
- no defense exists;
- current defense is weaker or narrower than the plan;
- current defense exists but has no good fault challenge;
- evidence is stale because tracked grounds changed;
- part of the promise has no coverage;
...
A current defense is a fact about the code. Evidence is a fact about a run that challenged that defense. A binding is a person's choice to rely on compatible supporting evidence.

These are separate because a passing suite can exist without testing the fault named by the promise. Upheld should ask what fault would show that the defense is inadequate and whether the defense can tell that fault from a valid state.
...
When code, a promise, a defense, or a tracked input changes, Upheld needs to know which parts of the map may need another look.

Doorstop, Boundver, build graphs, and other tools already solve parts of this lower-level job. Upheld can import their change signals where practical. A changed fingerprint means that old grounds need review. It does not mean the promise is false.
...
2. write or discover its promises;
...
Upheld should be judged by whether it helps teams make better choices about how promises are held.

Useful gains include finding promises that were never defended, finding tests that cannot catch the promised fault, suggesting a stronger or cheaper defense plan, showing uncovered scope, preserving good evidence across unrelated changes, and directing review only where recorded grounds changed.

The [milestone](MILESTONE.md) turns these goals into trials. The [component design](COMPONENT_REGISTER_DESIGN.md) carries the map into multi-component and cross-project work. The [method](METHOD.md) defines how a current defense earns evidence.
