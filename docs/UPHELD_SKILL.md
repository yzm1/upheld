...
Readers are engineers trying the Upheld guide. Version 0.1.0 is a packaged prototype dated 7 September 2026. It helps an agent review promises and evidence within a chosen scope. It uses your existing account and tools.
...
From an Upheld checkout, use Python 3.11 or later. Export uses the Python standard library. Name the new output folder `upheld`.
...
The manifest detects changed or missing files against the recorded hashes. It does not authenticate a publisher or prove that the method is current online. Keep the exported folder together; copying the entrypoint alone loses required rules.
